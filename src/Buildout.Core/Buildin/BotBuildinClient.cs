...
public sealed class BotBuildinClient : IBuildinClient
{
    ...
    public async Task<QueryDatabaseResult> QueryDatabaseAsync(string databaseId, QueryDatabaseRequest request, CancellationToken cancellationToken = default)
    {
        return await WrapAsync(async () =>
        {
            ...
            return MapQueryDatabaseResponse(result);
        });
    }
    ...
    private static Database MapDatabase(Gen.Database gen)
    {
        return new Database
        {
            ...
            Properties = MapDatabaseProperties(gen.Properties),
    ...
    private static Dictionary<string, PropertySchema>? MapDatabaseProperties(Gen.Database_properties? gen)
    {
        if (gen is null) return null;

        using var writer = new JsonSerializationWriter();
        writer.WriteObjectValue(null, gen);
        using var stream = writer.GetSerializedContent();
        using var doc = JsonDocument.Parse(stream);
    ...
    private static QueryDatabaseResult MapQueryDatabaseResponse(Gen.QueryDatabaseResponse? gen)
    {
