...
internal static partial class BotBuildinClientLog
{
    ...
    internal static partial void ApiCallCompleted(ILogger logger, string method, string outcome);

    [LoggerMessage(Level = LogLevel.Error, Message = "API call {Method} failed with {Outcome} error_type {ErrorType}")]
    internal static partial void ApiCallFailed(ILogger logger, string method, string outcome, string errorType);
}
