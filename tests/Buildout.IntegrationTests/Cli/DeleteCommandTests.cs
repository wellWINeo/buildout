...
public sealed class DeleteCommandTests
{
    ...
    public DeleteCommandTests(BuildinWireMockFixture fixture, ITestOutputHelper output)
    {
        _fixture = fixture;
        _output = output;
        _fixture.Reset();
    ...
    public async Task HappyPath_StateChange_Exits0()
    {
        ...
        Assert.Contains($"Deleted page {PageId}: archived=true (changed=true)", stdout.ToString());
    }
    ...
    public async Task HappyPath_NoOp_Exits0()
    {
        ...
        Assert.Equal(0, exitCode);
        Assert.Contains($"Deleted page {PageId}: archived=true (changed=false, no-op)", stdout.ToString());
    }
